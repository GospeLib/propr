...
import { consumeExecutionAdmission, createRedisAdmissionStore, pendingExecutionAdmissionKey, getAuthenticatedOctokit, issueQueue, generateCorrelationId, extractLlmFromLabels, logger } from '@propr/core';
...
export async function enqueueAdmittedComment(input: {
  repository: string; prNumber: number; commentId: number; body: string; admissionId: string;
  ...
  if (existing) {
    if (existing.data.executionAdmissionReceipt?.admissionId !== input.admissionId ||
  ...
  try {
    ...
    await issueQueue.add('processPullRequestComment', {
      ...
      correlationId: generateCorrelationId(), commandMode: 'default',
