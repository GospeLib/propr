...
function setupRoutes(): void {
  ...
  // EP-ezer-follow-ups-S03: Ezer control propagation. The stop propagator is
  // the same stopTaskExecution path the manual stop route uses (abort marker +
  // Docker container stop + queued job removal), and the stream projection is
  // resolved lazily because the socket service is created after setupRoutes().
  const ezerControlRoutes = createEzerControlRoutes({
    getStreamProjection: () => getSocketService()?.getEzerStreaming() ?? null,
    stopTask: (taskId, context) => stopTaskExecution(taskId, { redisClient, ...context }),
  });
  ...
  const operationalRoutes: RouteEntry[] = [
    ...
    ['post', '/api/ezer/operations/:operationId/attempts', ezerControlRoutes.registerAttempt], ['post', '/api/ezer/operations/:operationId/control', ezerControlRoutes.postControl], ['get', '/api/ezer/operations/:operationId/control', ezerControlRoutes.getControlState], ['post', '/api/ezer/operations/:operationId/steer/consume', ezerControlRoutes.consumeSteer],
  ];
  const routes = [
    ...operationalRoutes,
