...
const commonOptionPlacements = [
  {
    ...
    args: (
      subcommand: "repo" | "stack",
      commonOptions: string[],
      childOptions: string[] = [],
    ) => ["init", ...commonOptions, subcommand, ...childOptions],
  },
  {
    ...
    args: (
      ...
      commonOptions: string[],
      childOptions: string[] = [],
    ) => ["init", subcommand, ...childOptions, ...commonOptions],
  },
];
...
for (const placement of commonOptionPlacements) {
  test(`init repo --json emits parseable JSON with options ${placement.name}`, () => {
    ...
    try {
      const args = placement.args("repo", ["--json"]);
      ...
      const rerun = JSON.parse(runCli(args, repo, home)) as InitCommandResult;
  ...
  test(`init stack --json emits parseable JSON with options ${placement.name}`, () => {
    ...
    try {
      const args = placement.args("stack", ["--json"], ["--root", stack]);
  ...
  for (const placement of commonOptionPlacements) {
    test(
      ...
      () => {
        ...
        try {
          const stdout = runCli(
            placement.args("repo", ["--force", ...(json ? ["--json"] : [])]),
            repo,
            home,
          );
          ...
          assert.deepEqual(
            readdirSync(proprDir).sort(),
            [...repoScaffoldFiles, "sentinel.txt"].sort(),
          );
        } finally {
          rmSync(fixture, { recursive: true, force: true });
        }
  ...
  for (const placement of commonOptionPlacements) {
    test(
      ...
      () => {
        ...
        const home = join(fixture, "home");
        const oldEnv =
          "SESSION_SECRET=existing-stack-secret\nSELECTED_ROOT_MARKER=keep-in-backup\n";
        ...
        try {
          ...
          if (json) {
            ...
            assert.match(
              stdout,
              /Overwrote \.env from \.env\.example \(previous saved to \.env\.bak\)/,
            );
          }
        } finally {
          rmSync(fixture, { recursive: true, force: true });
