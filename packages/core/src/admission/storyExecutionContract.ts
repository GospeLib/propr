...
export function requireStoryExecutionContract(value: unknown): StoryExecutionContract {
  ...
  if (Object.keys(contract).some((key) => key !== 'recovery' && key !== 'taskAssignment' && !KEYS.includes(key as typeof KEYS[number])) ||
  ...
  return { ...(contract.recovery === undefined ? {} : { recovery: requireExecutionRecoveryContext(contract.recovery) }), ...(contract.taskAssignment === undefined ? {} : { taskAssignment: requireTaskAssignment(contract.taskAssignment) }), baseSha: contract.baseSha, featureBranch: contract.featureBranch,
