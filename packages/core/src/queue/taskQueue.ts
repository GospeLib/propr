...
type ExecutionQueueData = IssueJobData | CommentJobData | IntegrationJobData;
...
let _issueQueue: Queue<ExecutionQueueData> | null = null;
...
async function ensureInitialized(): Promise<void> {
    ...
    _issueQueue = new Queue<ExecutionQueueData>(GITHUB_ISSUE_QUEUE_NAME, issueQueueOptions);
 ...
 */
export async function getIssueQueue(): Promise<Queue<ExecutionQueueData>> {
...
export const issueQueue = new Proxy({} as Queue<ExecutionQueueData>, {
    get(_target, prop) {
        if (!_issueQueue) {
