...
test('preserves every partial change on a checkpoint ref without touching the feature branch or index', async () => {
...
test('a later attempt starts from the exact checkpoint in-scope work, uncommitted on the admitted base', async () => {
  ...
  assert.deepEqual(restored, { ref: record.ref, sha: record.sha,
    restoredPaths: ['allowed.txt', 'created.txt', 'removed.txt'], ignoredPaths: ['stray.log'] });
