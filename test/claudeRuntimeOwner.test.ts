...
import { resolveClaudeRuntimeOwner, resolveAgentRuntimeOwner } from '../packages/core/src/agents/impl/utils/dockerArgsBuilder.js';
import { setWorktreeOwnership } from '../packages/core/src/claude/claudeHelpers.js';
import { closeConnection } from '../packages/core/src/db/connection.js';

after(async () => { await closeConnection(); });

test('prepares Claude and Codex task worktrees for the actual mounted config owner', async () => {
  ...
  try {
    ...
    assert.deepEqual(resolveAgentRuntimeOwner(config), owner);
