...
for (const scope of ['typed', 'story', 'ordinary']) test(`preview prompt obeys ${scope} execution scope`, async () => {
 const scoped={...context,typedInvestigation:scope==='typed'?context.typedInvestigation:undefined,
   storyExecution:scope==='story'?{allowedPaths:['src/approved.ts']}:undefined,
   executionDeadline:new Date(Date.now()+60_000).toISOString()};
 await assert.rejects(executeAgentAndRecordMetrics({worktreeInfo:{worktreePath:'/unused',branchName:'branch'},issueRef:{repoOwner:'owner',repoName:'repo',number:1},githubToken:{token:'test'},currentIssueData:{data:{body:'',title:'',labels:[]}},issueComments:[]} as never,scoped as never),/Execution aborted by user request/);
 const prompt=(executeTask.mock.calls.at(-1)?.arguments[0] as {prompt:string}).prompt;
 if(scope==='ordinary')assert.match(prompt,/\.propr\/previews\//);
 else assert.doesNotMatch(prompt,/\.propr\/previews\//);
});
