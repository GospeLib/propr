...
describe('every paid provider call site is classified individually', () => {
    test('the ledger is derived from the sources, call site by call site', () => {
        assert.deepEqual(ledger, [
            ...
            'UNPROTECTED src/jobs/issueJob/agent.ts:199 agent.executeTask() in executeAgentAndRecordMetrics',
            ...
            'UNPROTECTED src/jobs/processTaskImportJob.ts:152 agent.executeTask() in processTaskImportJob',
...
describe('every process creation written in a recognised form is inventoried and classified', () => {
    ...
    test('the process-creation inventory is the one on file', () => {
        assert.deepEqual(repositoryProcessCreationSites().map(site =>
            ...
            'MODEL RUN executeDockerCommand packages/core/src/agents/impl/ClaudeAgent.ts:129 in ClaudeAgent.executeTask',
            'MODEL RUN executeDockerCommand packages/core/src/agents/impl/ClaudeAgent.ts:220 in ClaudeAgent.analyze',
            'management executeDockerCommand packages/core/src/agents/impl/ClaudeAgent.ts:308 in ClaudeAgent.healthCheck',
            ...
            'MODEL RUN executeDockerCommand packages/core/src/claude/claudeService.ts:113 in executeClaudeCode',
            'management child_process.execFile packages/core/src/claude/docker/dockerContainerControl.ts:47 in runDocker',
            'management executeDockerCommand packages/core/src/claude/docker/dockerExecutor.ts:114 in findTaskContainer',
            'management executeDockerCommand packages/core/src/claude/docker/dockerExecutor.ts:156 in inspectLegacyDockerContainerLivenessForTask',
            'management child_process.spawn packages/core/src/claude/docker/dockerExecutor.ts:183 in spawnCommandProcess',
            'management child_process.execFileSync packages/core/src/claude/docker/dockerExecutor.ts:421 in detectContainerId',
            ...
            'management child_process.execFile src/jobs/issueJob/agent.ts:4 in <module>',
